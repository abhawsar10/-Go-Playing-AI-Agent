...
class GO_BOARD:
    ...
    def place_n_check(self, i, j,piece_type, prints= False):
        ...
        test_board = deepcopy(test_go.board)
...
class myQlearner:
    ...
    def get_input(self, my_go_board, piece_type):
         
        move=my_go_board.score(1)+my_go_board.score(2)
        #print(move)
        """
        action = self.alpha_beta(my_go_board,2)
        """
        if move <=3:
            action = self.alpha_beta(my_go_board,2)
    ...
    def max_value(self, go, a, b, m, n, max_len, ply):
        ...
        if max_len>=ply:
            return m,n,self.state_utility(go.board, for_opponent= True )
        
        v = -np.inf
        
        safe_copy = deepcopy(go.board)
        go_max = GO_BOARD(5)
        go_max.board = deepcopy(go.board)
        ...
        for i in range(5):
            for j in range(5):
                if go_max.place_n_check(i, j, self.side):
    ...
    def min_value(self, go, a, b, i, j, max_len,ply):
        ...
        if max_len>=ply:
            return i,j,self.state_utility(go.board)
        
        v = +np.inf
        
        safe_copy = deepcopy(go.board)
        go_min = GO_BOARD(5)
        go_min.board = deepcopy(go.board)
        ...
        for m in range(5):
            for n in range(5):
                if go_min.place_n_check(m, n, 3- self.side):
    ...
    def state_utility(self, state, for_opponent = False, show_eval = False):
...
def display_winloss(winloss):
	...
	print("="*10+str(np.sum(winloss))+" Games Played"+"="*10);
...
if __name__ == "__main__":
	...
	parser.add_argument("-iter", type=int, help="iterations of training", default=20)
	parser.add_argument("-sturns", type=int, help="show turns", default=1)
