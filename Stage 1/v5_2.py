...
class myQlearner:
    ...
    def min_value(self, go, a, b, i, j, max_len,ply):
        ...
        if max_len>=ply:
            return i,j,self.state_utility(go.board, i, j)
    ...
    def state_utility(self, state, i, j, for_opponent = False, show_eval = False):
        ...
        d1,d2,d34,d5,d6 = 3,1,np.maximum(np.minimum((x+y)-10,10),1),3,np.maximum(10-(x+y),0)
        ...
        if show_eval:
            ...
            print()
            print("Close Allies       = "+str(d)+"  x"+str(d5)+"   = "+str(d5*d))
            print()
