...
class myQlearner:
    ...
    def search_u_table(self, go):
        ...
        for i  in range(len(diff_boards)):
            ...
            if i_board in self.Utable:
                return i,self.decode_board(i_board)
       
        return 0,self.decode_board(diff_boards[0])
    ...
    def get_input(self, go):
        ...
        transformation,u_list = self.search_u_table(go)
        
        mat = np.array(self.Uvals(u_list))
